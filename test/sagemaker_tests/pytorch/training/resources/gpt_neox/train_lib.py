...
def finetune_with_pretrained_weights_check(args):
...
def eval_model(model, data_pipeline, num_batches):
    ...
    with torch.no_grad():
        for batch_idx, input_data in enumerate(data_pipeline.val_dataloader):
            input_ids, mask = data_pipeline.get_val_batch(input_data)
...
def train_step(
    ...
    input_ids, mask, labels = data_pipeline.get_batch(input_data)
...
def train(
    ...
    while total_steps < args.max_steps:
        nvtx_warmup_iters = 3
        if global_rank == 0:
            logger.info(f"Starting training with epoch {epoch}")
        ...
        for batch_idx, input_data in enumerate(data_pipeline.train_dataloader):
            if global_rank == 0:
                logger.info(f"Batch idx {batch_idx}, total_steps {total_steps}, max_steps {args.max_steps}")
...
def main(args):
    ...
    if tsm.state.tensor_parallel_degree > 1 and args.use_smp_implementation < 1:
        args.use_smp_implementation = 1
        if global_rank == 0:
            logger.info(
                f"Tensor parallelism (TP) is enabled as tensor_parallel_degree is set to {tsm.state.tensor_parallel_degree} (>1). Switching use_smp_implementation to 1 so we can use SMP optimized implementation for TP."
            )
    ...
    with tsm_utils.timeit(True, "Model creation", global_rank):
        ...
        if args.use_smp_implementation:
            load_state_dict_from_rank0 = (
                finetune_with_pretrained_weights_check(args)
            )
    ...
    if tsm.state.sm_activation_offloading:
    ...
    if args.resume_from_checkpoint:
        (
            ...
            sharding_strategy=sharding_strategy,
            checkpointing_pg_metadata=checkpointing_pg_metadata,
            tensor_parallel_degree=int(tsm.state.tensor_parallel_degree),
            checkpoint_type=args.checkpoint_type,
        )
